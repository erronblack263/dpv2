        ...
        <div>
          ...
          <div className="mb-8">
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-sm font-semibold text-primary uppercase tracking-widest">Featured Projects</span>
            </div>
            <div className="grid gap-6 lg:grid-cols-2">
              {featured.map((f) => (
                <div key={f.title} className="group relative overflow-hidden rounded-3xl border border-border bg-card transition-all hover:border-primary/40 hover:shadow-xl">
                  {/* Illustration */}
                  <div className={`relative h-36 bg-gradient-to-br ${f.gradient} overflow-hidden flex items-center justify-center`}>
                    <div className="absolute top-4 left-4 size-24 rounded-full blur-3xl" style={{ background: f.svgColor + '33' }} />
                    <div className="absolute bottom-4 right-4 size-16 rounded-full blur-2xl" style={{ background: f.svgColor + '22' }} />
                    <span className="text-5xl">{f.title.includes('GreenSpace') ? '🌱' : '🛡️'}</span>
                  </div>
                  {/* Info */}
                  <div className="flex flex-col justify-between p-5">
                    <div>
                      <h2 className="text-xl font-extrabold tracking-tight">{f.title}</h2>
                      <p className={`mt-2 text-sm text-muted-foreground leading-relaxed ${descExpanded ? '' : 'line-clamp-3'}`}>
                        {f.description}
                      </p>
                      <button
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                      <a href={f.demo} className="flex items-center gap-1.5 rounded-2xl bg-primary text-primary-foreground px-4 py-2 text-xs font-semibold transition-colors hover:bg-primary/90">
                        <ExternalLink className="size-3.5" /> Live Demo
                      </a>
                      <a href={f.github} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 rounded-2xl border border-border bg-background px-4 py-2 text-xs font-semibold transition-colors hover:bg-accent">
                        <Code2 className="size-3.5" /> GitHub
                      </a>
                      {f.artifacts !== '#' && (
                        <a href={f.artifacts} className="flex items-center gap-1.5 rounded-2xl border border-border bg-background px-4 py-2 text-xs font-semibold transition-colors hover:bg-accent">
