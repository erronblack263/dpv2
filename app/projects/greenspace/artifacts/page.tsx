...
import { ArrowLeft, Trophy, X, ChevronLeft, ChevronRight, ChevronDown } from 'lucide-react'
...
function SectionGallery({ section, globalOffset, onOpen }: {
  section: SectionType
  globalOffset: number
    ...
    <div className="mb-4 overflow-hidden rounded-2xl border border-border bg-card">
      ...
      >
        <div className="flex items-center gap-3">
          <h2 className="text-lg font-extrabold tracking-tight text-foreground">{section.title}</h2>
          <span className="rounded-full bg-primary/10 px-2.5 py-0.5 text-xs font-semibold text-primary">{total} images</span>
        </div>
...
export default function GreenSpaceArtifactsPage() {
  ...
  function prev() { setLightbox((i) => (i !== null ? (i - 1 + allImages.length) % allImages.length : null)) }
  function next() { setLightbox((i) => (i !== null ? (i + 1) % allImages.length : null)) }
      ...
      <section className="mx-auto w-full max-w-6xl px-4 pt-12 pb-24 sm:px-6">
        ...
        <div className="mb-10 flex items-start gap-4">
          ...
          <div>
            ...
            <p className="mt-2 text-muted-foreground">Click a section to expand. Tap any image to view full size.</p>
          </div>
        </div>
        ...
        {sections.map((section) => {
          ...
          return <SectionGallery key={section.title} section={section} globalOffset={sectionOffset} onOpen={setLightbox} />
        })}
      </section>
